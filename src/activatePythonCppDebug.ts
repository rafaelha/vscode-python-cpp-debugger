...
export function activatePythonCppDebug(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "extension.pythonCpp-debug.runEditorContents",
      (resource: vscode.Uri) => {
    ...
    vscode.commands.registerCommand(
      "extension.pythonCpp-debug.debugEditorContents",
      (resource: vscode.Uri) => {
        let targetResource = resource;
        if (!targetResource && vscode.window.activeTextEditor) {
          targetResource = vscode.window.activeTextEditor.document.uri;
  ...
  context.subscriptions.push(
    vscode.debug.registerDebugConfigurationProvider("pythoncpp", provider)
  );
  ...
  context.subscriptions.push(
    vscode.debug.registerDebugAdapterDescriptorFactory("pythoncpp", factory)
  );
  if ("dispose" in factory && typeof factory.dispose === "function") {
    context.subscriptions.push({ dispose: factory.dispose.bind(factory) });
  }
}
